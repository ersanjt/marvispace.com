/**
 * Floating WhatsApp support button (loads config from public settings API).
 * @project MARVISPACE
 */
...
function buildWhatsAppUrl(config, customText = '') {
  if (!config?.phone) return '';
  const text = (customText || config.message || '').trim();
  const base = `https://wa.me/${config.phone}`;
  return text ? `${base}?text=${encodeURIComponent(text)}` : base;
}

function renderButton(url) {
  if (!url || document.querySelector('.whatsapp-support')) return;
...
export async function initWhatsAppSupport(options = {}) {
  if (mounted || document.body.classList.contains('admin-app')) return;
  mounted = true;
